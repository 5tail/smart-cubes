...
> ⚠️ **狀態（v0.1.0 前）**：GAN 與 MoYu WeiLong AI 已實機驗收通過；QiYi 標準版
> （QY-QYSC）仍待實機驗證，故尚未發佈到 npm。詳見 [已驗證型號](#已驗證型號)。
...
> 尚未發佈到 npm（等 QiYi 標準版實機驗過再發 0.1.0）。現在要試可直接以 git 依賴安裝，
...
| **QiYi** | QiYi AI 3x3 標準版（`QY-QYSC…`） | ⏳ 已實作、**待實機驗證**（發 0.1.0 前的最後一關） |
| QiYi | Tornado V4 LE（`XMD-TornadoV4LE…`） | ❌ **已知不支援**：driver 會辨識其名稱前綴，但目前協議實作無法正確驅動；勿當可用型號 |
...
> Not on npm yet — 0.1.0 ships once the standard QiYi (QY-QYSC) is verified on real
> hardware. Until then, install as a git dependency or run the local [demo](#demo).
...
| **QiYi** | QiYi AI 3x3 standard (`QY-QYSC…`) | ⏳ implemented, **pending hardware verification** (last gate before 0.1.0) |
| QiYi | Tornado V4 LE (`XMD-TornadoV4LE…`) | ❌ **known unsupported**: the driver matches its name prefix but the current protocol does not drive it correctly — do not treat as usable |
