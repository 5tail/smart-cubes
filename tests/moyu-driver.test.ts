...
describe('connectMoyuDevice — 金鑰自動探測（連上後試候選 MAC，用能解出合法封包的那組）', () => {
  ...
  it('探測都沒過但名稱可推導 → 用名稱推導直接連上，且不跳手動輸入框', async () => {
    // 名稱推導金鑰 ≠ fixture 金鑰 → 探測失敗；無廣播、無 macProvider。
    const manualSpy = vi.fn(() => Promise.resolve('CF:30:16:00:11:22'));
    const { device, disconnectSpy } = makeProbeMock('WCU_MY32_ABCD');
    const driver = await connectMoyuDevice(
      device,
      { macProvider: (_d, fb) => (fb ? manualSpy() : Promise.resolve(null)) },
      20,
    );
    expect(driver.mac).toBe('CF:30:16:00:AB:CD'); // 名稱推導
    expect(driver.macSource).toBe('name');
    expect(manualSpy).not.toHaveBeenCalled(); // 關鍵：魔域不跳手動 MAC 輸入框
    expect(disconnectSpy).not.toHaveBeenCalled(); // 有候選就連上（交看門狗判斷），不斷線
    await driver.disconnect();
  });

  it('完全無候選 MAC（名稱不可推導 + 無廣播）→ 斷開 GATT 並拋錯', async () => {
    const { device, disconnectSpy } = makeProbeMock('WCU_MY32_ZZZZ'); // 非 hex 後綴 → 名稱推導 null
    await expect(connectMoyuDevice(device, {}, 20)).rejects.toThrow(/需要 MAC/);
    expect(disconnectSpy).toHaveBeenCalled(); // 釋放 GATT，方塊才能再度廣播
  });
});
